@extends('tenant.frontend.layouts.index')
    ...
    @if ($featuredNews->count() > 0)
        <section class="mb-12">
            <div class="grid md:grid-cols-3 gap-6">
                ...
                <article
                    class="md:col-span-2 bg-white rounded-xl shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-300">
                    @if ($featuredNews[0]->featured_image)
                        <div class="relative overflow-hidden h-96">
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-8">
                                <span
                                    class="inline-block px-4 py-2 bg-red-600 text-white rounded-full text-sm font-bold uppercase mb-4">
                                    {{ $featuredNews[0]->category->name }}
                                </span>
                                <h2 class="text-3xl md:text-4xl font-bold text-white mb-3 leading-tight">
                                    <a href="{{ route('tenant.news.public', $featuredNews[0]->slug) }}"
                ...
                <div class="space-y-6">
                    @foreach ($featuredNews->slice(1, 2) as $news)
                        <article
                            ...
                            <div class="p-5">
                                <span
                                    class="inline-block px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold uppercase mb-2">
                                    {{ $news->category->name }}
                                </span>
                                <h3
                        ...
                        @foreach ($latestNews as $news)
                            <article
                                ...
                                <div class="md:flex">
                                    @if ($news->featured_image)
                                        <div class="md:w-1/3 relative overflow-hidden">
                                            ...
                                            <div class="absolute top-4 left-4">
                                                <span
                                                    class="px-3 py-1 bg-{{ $news->category->color ?? 'blue' }}-600 text-white rounded-full text-xs font-bold uppercase shadow-lg">
                                                    {{ $news->category->name }}
                                                </span>
                                            </div>
                                        </div>
                                    ...
                                    <div
                                        class="p-6 {{ $news->featured_image ? 'md:w-2/3' : 'w-full' }} flex flex-col justify-between">
                                        <div>
                                            <div class="flex items-center gap-3 mb-3">
                                                @if (!$news->featured_image)
                                                    <span
                                                        class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold uppercase">
                                                        {{ $news->category->name }}
                                                    </span>
                                                @endif
                                            ...
                                            <h3
                                                class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors leading-tight">
                                                <a href="{{ route('tenant.news.public', $news->slug) }}">
                                                    {{ $news->title }}
                                                </a>
                                            </h3>
        ...
        <aside class="space-y-8">
                        ...
                        <li>
                            <a href="{{ route('tenant.category.news', $category->slug) }}"
                                ...
                                <div class="flex items-center gap-2">
                                    <span
                                        class="px-2 py-1 bg-gray-100 group-hover:bg-blue-100 text-gray-700 group-hover:text-blue-600 rounded-full text-xs font-bold transition-colors">
                                        {{ $category->news_count }}
                                    </span>
                ...
                <ul class="space-y-4">
                    @foreach ($latestNews->take(5) as $index => $news)
                        <li class="flex gap-3">
                            ...
                            <div>
                                <a href="{{ route('tenant.news.public', $news->slug) }}"
                                    class="text-gray-900 hover:text-red-600 font-medium line-clamp-2 transition-colors">
                                    {{ $news->title }}
                                </a>
                                <span
                                    class="text-xs text-gray-500 mt-1 block">{{ $news->published_at->diffForHumans() }}</span>
                            </div>
                        </li>
                    @endforeach
                </ul>
            ...
            <div class="bg-white rounded-xl shadow-lg p-6">
                ...
                <p class="text-gray-600 leading-relaxed text-sm mb-4">
                    Welcome to <strong>{{ tenant('name') ?? 'our news portal' }}</strong>. We bring you the latest news and
                    updates
